...
func (im *Image) Invoke(ctx context.Context, req *models.InvokeRequest) (*types.ModelResponse, error) {
    ...
    resp, err := provimg.Generate(ctx, im.client, provimg.Options{Prompt: prompt, Size: size, N: n, References: refs, MaskURL: mask, CallbackURL: cb})
    if err != nil { return nil, err }
...
func stringSliceFromExtra(req *models.InvokeRequest, key string) []string {
    if req != nil && req.Extra != nil {
        if v, ok := req.Extra[key]; ok {
            ...
            case []interface{}:
                var out []string
                for _, it := range vv { if s, ok := it.(string); ok { out = append(out, s) } }
                return out
            }
        }
    }
