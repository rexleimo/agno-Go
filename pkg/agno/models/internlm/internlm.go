...
// InternLM provider (configurable OpenAI-compatible endpoint)
type InternLM struct {
    models.BaseModel
...
func New(modelID string, config Config) (*InternLM, error) {
    if config.APIKey == "" { return nil, types.NewInvalidConfigError("InternLM API key is required", nil) }
    if config.BaseURL == "" { return nil, types.NewInvalidConfigError("InternLM BaseURL is required", nil) }
    cc := openai.DefaultConfig(config.APIKey); cc.BaseURL = config.BaseURL
    to := config.Timeout; if to == 0 { to = 60 * time.Second }
    cc.HTTPClient = &http.Client{Timeout: to}
    return &InternLM{ BaseModel: models.BaseModel{ID: modelID, Provider: "internlm"}, client: openai.NewClientWithConfig(cc), config: Config{APIKey: config.APIKey, BaseURL: config.BaseURL, Temperature: config.Temperature, MaxTokens: config.MaxTokens, Timeout: to} }, nil
}

func (in *InternLM) Invoke(ctx context.Context, req *models.InvokeRequest) (*types.ModelResponse, error) {
    chatReq := in.buildChatRequest(req)
    resp, err := in.client.CreateChatCompletion(ctx, chatReq)
    if err != nil { return nil, types.NewAPIError("failed to call InternLM API", err) }
    if len(resp.Choices) == 0 { return nil, types.NewAPIError("no response from InternLM", nil) }
    ch := resp.Choices[0]
...
func (in *InternLM) InvokeStream(ctx context.Context, req *models.InvokeRequest) (<-chan types.ResponseChunk, error) {
    chatReq := in.buildChatRequest(req); chatReq.Stream = true
    stream, err := in.client.CreateChatCompletionStream(ctx, chatReq)
...
func (in *InternLM) buildChatRequest(req *models.InvokeRequest) openai.ChatCompletionRequest {
    chatReq := openai.ChatCompletionRequest{ Model: in.ID, Messages: make([]openai.ChatCompletionMessage, len(req.Messages)) }
    ...
    temp, max := models.MergeConfig(req.Temperature, in.config.Temperature, req.MaxTokens, in.config.MaxTokens)
