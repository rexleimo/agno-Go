...
func New(modelID string, config Config) (*Portkey, error) {
    if config.APIKey == "" { return nil, types.NewInvalidConfigError("Portkey API key is required", nil) }
    ...
    return &Portkey{ BaseModel: models.BaseModel{ID: modelID, Provider: "portkey"}, client: openai.NewClientWithConfig(cc), config: Config{APIKey: config.APIKey, BaseURL: baseURL, Temperature: config.Temperature, MaxTokens: config.MaxTokens, Timeout: to}}, nil
}

func (p *Portkey) Invoke(ctx context.Context, req *models.InvokeRequest) (*types.ModelResponse, error) {
    chatReq := p.buildChatRequest(req)
    resp, err := p.client.CreateChatCompletion(ctx, chatReq)
    if err != nil { return nil, types.NewAPIError("failed to call Portkey API", err) }
    if len(resp.Choices) == 0 { return nil, types.NewAPIError("no response from Portkey", nil) }
    ch := resp.Choices[0]
...
func (p *Portkey) InvokeStream(ctx context.Context, req *models.InvokeRequest) (<-chan types.ResponseChunk, error) {
    chatReq := p.buildChatRequest(req); chatReq.Stream = true
...
func (p *Portkey) buildChatRequest(req *models.InvokeRequest) openai.ChatCompletionRequest {
    chatReq := openai.ChatCompletionRequest{ Model: p.ID, Messages: make([]openai.ChatCompletionMessage, len(req.Messages)) }
    ...
    temp, max := models.MergeConfig(req.Temperature, p.config.Temperature, req.MaxTokens, p.config.MaxTokens)
